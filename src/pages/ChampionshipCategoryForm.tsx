...
const ChampionshipCategoryForm: React.FC = () => {
  ...
  const { fightModalities: modalitiesFromContext = [] } = useFightModalities()
  
  // Fallback sample modalities if context is not working
  const sampleModalities = [
    {
      modalityId: 'MOD001',
      name: 'Brazilian Jiu-Jitsu',
      description: 'Ground fighting and submission grappling martial art',
      type: 'grappling' as const,
    ...
    },
    {
      ...
      description: 'Ground fighting without traditional uniform',
      type: 'grappling' as const,
      level: 'intermediate' as const,
      duration: 90,
      active: true,
  ...
  // Use context modalities if available, otherwise use sample data
  const modalities = modalitiesFromContext.length > 0 ? modalitiesFromContext : sampleModalities
  
  // Debug: Check modalities loading
  console.log('ChampionshipCategoryForm - Context Modalities:', modalitiesFromContext)
  console.log('ChampionshipCategoryForm - Using Modalities:', modalities)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {modalities.length > 0 ? modalities.map(modality => (
                    ...
                    <div className="col-span-2 text-center py-8">
                      ...
                      <p className="text-gray-400 text-lg mb-2">No fight modalities available</p>
                      <p className="text-gray-500 text-sm">Please create fight modalities first in the Fight Plans section</p>
                      <div className="mt-4 p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
                        <p className="text-yellow-300 text-sm">Debug: Modalities count: {modalities.length}</p>
